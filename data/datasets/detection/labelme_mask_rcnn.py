...
from data.datasets.detection.labelme_base import LabelMeDetection
from data.transforms import image_pil as T
from data.transforms.common import Compose


@DATASET_REGISTRY.register(name="labelme_mask_rcnn", type="detection")
class LabelMeDetectionMaskRCNN(LabelMeDetection):
    """Dataset class for the LabelMe Object Detection using Mask RCNN .
    ...
    def __init__(self, opts: argparse.Namespace, *args, **kwargs) -> None:
        ...
        setattr(opts, "dataset.collate_fn_name_train", "labelme_mask_rcnn_collate_fn")
        setattr(opts, "dataset.collate_fn_name_val", "labelme_mask_rcnn_collate_fn")
        setattr(opts, "dataset.collate_fn_name_test", "labelme_mask_rcnn_collate_fn")

    @classmethod
    def add_arguments(cls, parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
        if cls != LabelMeDetectionMaskRCNN:
            # Don't re-register arguments in subclasses that don't override `add_arguments()`.
            return parser

        group = parser.add_argument_group(title=cls.__name__)
        group.add_argument(
            "--dataset.detection.labelme-mask-rcnn.use-lsj-aug",
            action="store_true",
    ...
    def _training_transforms(
        ...
        if getattr(self.opts, "dataset.detection.labelme_mask_rcnn.use_lsj_aug"):
...
@COLLATE_FN_REGISTRY.register(name="labelme_mask_rcnn_collate_fn")
def labelme_mask_rcnn_collate_fn(
    ...
    **kwargs
    ...
    For expected keys, see the keys in the output of `__getitem__` function of LabelMeDetectionMaskRCNN class.
